...
def train_model(X_train: np.array,
    ...
    params_grid = _build_param_grid(
        modelling_params['rf_params_grid'],
        'random_forest')
...
def evaluate_model_by_slices(X: pd.DataFrame,
    ...
    xtab_filtered = xtab[(xtab['group_size'] >= 100) & (
        xtab['prev'] >= 0.1)].reset_index(drop=True).copy()
    ...
    majority_bdf = b.get_disparity_major_group(
        xtab_filtered, original_df=df_aq)
